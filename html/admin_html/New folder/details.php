   <div class="col-sm-11 col-xs-12">
                <div class="col-xs-12 r-side-table">
                    <div class="col-sm-9">
                        <h4> <span> <i class="fa fa-users" aria-hidden="true"></i></span> شؤون الاسرة </h4>
                    </div>
                    <div class="col-sm-3">
                        <h5> موظف استقبال </h5>
                        <h5>   اخر دخول  : 27 / 5 / 2017</h5>
                    </div>
                </div>
                <div class="col-xs-12 r-bottom">
                    <div class="panel-heading">
                        <ul class="nav nav-tabs">
                            <li class="active"><a href="details.html"> الاسرة الجديده </a></li>
                            <li><a href="details1.html">الاسرة المعتمده </a></li>
                            <li><a href="details2.html">ارشيف الاسر </a></li>
                            <li><a href="details3.html"> خدمات الاسر </a></li>
                            <li><a href="details4.html"> خدمات معتمده </a></li>
                            <li><a href="details5.html">  ارشيف خدمات </a></li>
                        </ul>
                    </div>
                    <div class="panel-body">
                        <div class="fade in active">
                            <table id="example" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        <th class="text-center">م</th>
                                        <th class="text-center">الاسم</th>
                                        <th class="text-center">رقم الطلب </th>
                                        <th class="text-center">رقم الهوية</th>
                                        <th class="text-center">عرض</th>
                                        <th class="text-center last-th">حاله الاسرة </th>
                                    </tr>
                                </thead>
                                <tbody class="text-center">
                                    <tr>
                                        <td>1</td>
                                        <td>احمد</td>
                                        <td>4815</td>
                                        <td>8516515</td>
                                        <td><a href="inner-details.html"><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>2</td>
                                        <td>محمد</td>
                                        <td>9856</td>
                                        <td>8516515</td>
                                        <td><a href="inner-details1.html"><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>3</td>
                                        <td>اشرف</td>
                                        <td>1235</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>4</td>
                                        <td>رجب</td>
                                        <td>7855</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>5</td>
                                        <td>بهاء</td>
                                        <td>5123</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>6</td>
                                        <td>احمد</td>
                                        <td>4815</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>7</td>
                                        <td>محمد</td>
                                        <td>9856</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>8</td>
                                        <td>اشرف</td>
                                        <td>1235</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>9</td>
                                        <td>رجب</td>
                                        <td>7855</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>10</td>
                                        <td>بهاء</td>
                                        <td>5123</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>
                                    <tr>
                                        <td>11</td>
                                        <td>بهاء</td>
                                        <td>5123</td>
                                        <td>8516515</td>
                                        <td><a href=""><i class="fa fa-search-plus" aria-hidden="true"></i></a></td>
                                        <td>دراسة الطلب عند موظف </td>
                                    </tr>

                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
